...
- **Publish no longer corrupts forms (0x80048425).** `RegisterEvent` decorations are validated before any form is touched (GUID form/trigger ids, `onload`/`onsave` only, non-empty function); a bad decoration stops the write with a list of problems instead of sending Dataverse an invalid form XML.
